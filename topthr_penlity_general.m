...
function [y, loop, loop_k, c, x, energies, energies_k]= ...
    topthr_penlity_general(nelx0, nely0, lambda, r0, volfrac, frac, p, g, sd, objectfunc, bc, w,continuation, x, fileID,logtype, V_constrain, descent_type)
...
while 1
    ...
    if loop == 1
        [UV,c] = solver_elasticity_Q1(xPhys,F,H,freedofs,p);
    ...
    gk = -dY(E0,Emin,xPhys,p).*UV; %E_0^{-1}\sigma:\varrho = 1/A(\chi)^2 Ee(u):e(v)
    % filtering
    if sd > 0 
...
%         end
        ...
        [UV,c] = solver_elasticity_Q1(xPhys_new,F,H,freedofs,p); %计算试探的目标函数值
...
[~,c] = solver_elasticity_Q1(x,F,H,freedofs,p);
loop = loop + 1;
